...
export default function Jornal() {
  return (
    <section id="jornal" className="relative py-32 lg:py-48 px-6 lg:px-16 border-t border-terra/30">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-16 lg:mb-24">
          <div className="space-y-4">
            <p className="label-eyebrow text-terra text-[0.7rem]">04 — Editorial</p>
            <h2 className="font-serif-editorial text-5xl md:text-7xl text-bone">
              Jornal
            </h2>
          </div>
          <a href="#" className="label-eyebrow editorial-link self-start md:self-auto text-[0.7rem]">
          ...
          {journalEntries.map((j, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-3">
                <span className="label-eyebrow text-terra">{j.date}</span>
                <span className="w-6 h-px bg-terra" />
                <span className="label-eyebrow text-terra">{j.readTime}</span>
              </div>
              ...
              <p className="mt-3 text-bone-dim leading-relaxed text-sm">
                {j.excerpt}</p>
                <span className="inline-block mt-5 label-eyebrow editorial-link text-[0.7rem]">
                  Ler →
                </span>
              </Link>
          ))}
